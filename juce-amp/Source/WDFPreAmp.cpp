...
StageVoltages WDFPreAmp::processStageU1(double sample)
{
...
StageVoltages WDFPreAmp::processStageU2(double sample)
{
...
double WDFPreAmp::processPreOut(double sample)
{
...
double WDFPreAmp::process(double sample)
{
	auto u1 = processStageU1(sample);
	double u1Grid = u1.grid;
	double u1Cathode = u1.cathode;

	double u1Plate = triodeNN.process(u1Grid - u1Cathode);

	auto u2 = processStageU2(u1Plate);
	double u2Grid = u2.grid;
	double u2Cathode = u2.cathode;

	double u2Plate = triodeNN.process(u2Grid-u2Cathode);
	//DBG(u1Plate);

	return processPreOut(u1Plate);
